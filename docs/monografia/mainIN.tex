...
\renewcommand*{\bibfont}{\scriptsize}   % o \tiny si prefieres más compacto
...
\definecolor{colHeader}{RGB}{0,90,160}
...
\newcommand{\customframetitle}{%
  ...
  \begin{beamercolorbox}[wd=\paperwidth,ht=1.5ex,dp=0ex,leftskip=0ex,rightskip=1.3ex]{frametitle}%
...
\begin{itemize}
  \item \textbf{EEGNet}: CNN compacta con convoluciones separables, generaliza con pocos datos {\tiny\cite{Lawhern2018}}
  \item \textbf{Shallow ConvNet}: Arquitectura superficial para ritmos mu/beta, baseline reproducible en BCI {\tiny\cite{Schirrmeister2017}}
  \item \textbf{Demencia + conectividad}: Shallow CNN con AEC/PLV para AD vs FTD vs controles, alta exactitud {\tiny\cite{Ajra2023}}
  \item \textbf{Brecha}: Falta conexión directa entre deep-EEG y biomarcadores tau {\tiny\cite{dePaula2009}}
\end{itemize}
\end{frame}
...
\begin{frame}{Novelty}
\begin{itemize}
  \item \textbf{Innovación}: Unir neurobiología de tau con detección funcional en EEG mediante CNN compactas {\tiny\cite{Lawhern2018,Ajra2023}}
  \item \textbf{Etiqueta del estudio}: Biomarcador EEG funcional para progresión de Alzheimer
  \item \textbf{Métricas}: Accuracy, F1 macro, AUC ROC, \(P_D\), \(P_{FA}\); mapas de importancia por canal/banda {\tiny\cite{Kay1998}}
\end{itemize}
\end{frame}
